...
```
 ...
 (or)  UI upload / Batch ingest  ─────────┤
   ...
   (Claude)  (PO +    (6 checks →  (pure,    (race-safe   (append-only,
...
4. **Validate** — 6 checks (po_lookup, vendor_approved, po_status, total_tolerance,
   line_reconciliation, duplicate) produce an **evidence report — never a verdict**.
...
| **0:30–2:00** | **Run view** → drag **`demo_approve_dell.pdf`** → Process → 7 stages animate → APPROVE card | "I drop in a Dell invoice. Watch the real stages: it's *read* by Claude — that's the only AI step — then we find the PO, check the vendor's approved, validate the amounts and line items, and *decide*. APPROVE. The reason and these six checks are generated by a deterministic engine, not the model — so the decision is reproducible and explainable. Here's the source PDF it read." (Expand "What the system saw".) |
...
**If you have a spare beat** (cut if over time): re-drag a file you already processed →
**REJECT (duplicate)** — "and it won't pay the same invoice twice."
...
The verified live-upload trio (`data/demo_live/`, regenerated by
`scripts/make_live_demo_invoices.py`): **Dell → APPROVE**, **Globex → REJECT**,
**TechGear → FLAG** — all confirmed end-to-end through the real pipeline.
